...
def main():
    ...
    parser.add_argument(
        "--protocol",
        choices=("rule-first", "pure-llm", "native-exact"),
        default="rule-first",
        help="Scoring path: fast exact checks plus LLM, LLM on every row, or exact matching only",
    )
    parser.add_argument("--answer_path", default=None, help="Override model answer JSONL path")
    parser.add_argument("--save_path", default=None, help="Override judge JSON path")
    args = parser.parse_args()

    if args.protocol != "native-exact" and not args.api_base and not args.judge_model_path:
        print(
    ...
    answer_path = args.answer_path or f"model_answer/{args.benchmark}/{args.model}_answer.jsonl"
    save_path = args.save_path or f"judge/{args.benchmark}/{args.model}_answer.jsonl"
    ...
    for i, item in enumerate(tqdm(data_list, desc="Rule-based grading")):
        ...
        if args.protocol == "native-exact":
            if is_mcq:
                native_correct = first_letter_match(gt, extracted_answer)
            elif is_pope:
                native_correct = pope_extract(extracted_answer) == gt.strip().lower()
            elif is_mmvp:
                native_correct = mmvp_extract(extracted_answer) == gt.strip().lower()
            else:
                native_correct = extracted_answer.strip() == gt.strip()
            item["judge"] = "Yes" if native_correct else "No"
            item["judge_source"] = "native_exact"
            continue
    ...
    print(f"Protocol: {args.protocol}; total: {len(data_list)}, LLM used: {len(prompt_lists)}")
