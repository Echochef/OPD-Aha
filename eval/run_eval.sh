...
JUDGE_PROTOCOL="${JUDGE_PROTOCOL:-rule-first}"
JUDGE_OUT_DIR="${JUDGE_OUT_DIR:-judge/${JUDGE_PROTOCOL}}"
...
run_single_benchmark() {
  ...
  local judge_json="${JUDGE_OUT_DIR}/${bench}/${model_tag}_answer.json"

  python3 judge_qwenlm.py \
    --benchmark "${judge_benchmark}" \
    --model "${judge_model_tag}" \
    --protocol "${JUDGE_PROTOCOL}" \
    --answer_path "${OUT_DIR}/${bench}/${model_tag}_answer.jsonl" \
    --save_path "${judge_json}" \
    "${JUDGE_ARGS[@]}"
