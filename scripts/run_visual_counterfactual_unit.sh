...
TEACHER_REGULARIZATION="${TEACHER_REGULARIZATION:-frozen}"
TEACHER_UPDATE_RATE="${TEACHER_UPDATE_RATE:-0.0}"
COUNTERFACTUAL_NULL_MODE="${COUNTERFACTUAL_NULL_MODE:-mean_color}"
COUNTERFACTUAL_EXTRAPOLATION_BETA="${COUNTERFACTUAL_EXTRAPOLATION_BETA:-1.0}"
...
EXPERIMENT_NAME="${EXPERIMENT_NAME_OVERRIDE:-Vision-Counterfactual-Unit-${MODEL_NAME}}"
...
if [[ -n "$TRAINER_TOTAL_TRAINING_STEPS" ]]; then
    TOTAL_STEPS_ARGS+=(trainer.total_training_steps="$TRAINER_TOTAL_TRAINING_STEPS")
fi
...
echo "Counterfactual null mode: $COUNTERFACTUAL_NULL_MODE"
echo "Counterfactual extrapolation beta: $COUNTERFACTUAL_EXTRAPOLATION_BETA"
...
python3 -m verl.trainer.main_ppo --config-name "$CONFIG_NAME" \
    ...
    actor_rollout_ref.actor.self_distillation.counterfactual_null_mode=$COUNTERFACTUAL_NULL_MODE \
    actor_rollout_ref.actor.self_distillation.counterfactual_extrapolation_beta=$COUNTERFACTUAL_EXTRAPOLATION_BETA \
