...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        has_teacher_null_multi_modal_inputs = self._has_non_empty_multi_modal_inputs(
            data.non_tensor_batch.get("teacher_null_multi_modal_inputs")
        )
                ...
                for micro_batch in micro_batches:
                    ...
                    if self_distillation_enabled:
                        ...
                        if counterfactual_null_mode is not None:
                            ...
                            teacher_null_inputs = dict(teacher_inputs)
                            teacher_null_inputs["multi_modal_inputs"] = model_inputs[
                                "teacher_null_multi_modal_inputs"
                            ]
                            with torch.no_grad():
                            ...
                            stage_wall_time_totals[
                                "timing_s/update_actor/teacher_null_forward"
                            ] += teacher_null_forward_time
                            teacher_null_all_logps = (
                                teacher_null_outputs.get("all_logps") if return_all_logps else None
                            )
